    ...
    private sealed class InternalThreadLoop : IDisposable
    {
        ...
        public unsafe InternalThreadLoop(KcpContext* context)
        {
            ...
            _thread = new Thread(DoThreadLoop)
            {
                ...
                Name = $"KCP Protocol Thread Loop #{InterlockedHelper.Increment(ref _idCounter):D}"
            };
            _thread.Start();
        }
        ...
        public void Flush(bool blocking)
        {
            if (blocking)
            {
                ...
                InterlockedHelper.CompareExchange(ref _needManuallyFlushing, 1UL, 0UL);
                awaiter.Task.Wait();
            }
            else
            {
        ...
        public bool Receive(Span<byte> destination, out int bytesWritten)
        {
            ...
            bytesWritten = 0;
            while (Volatile.Read(ref _disposed) == 0UL)
            {
        ...
        private unsafe void DoThreadLoop()
        {
            ...
            while (Volatile.Read(ref _disposed) == 0UL)
            {
                ...
                if (InterlockedHelper.Exchange(ref _needManuallyFlushing, 0UL) != 0UL)
                {
                    Thread.MemoryBarrier();
        ...
        {
            if (InterlockedHelper.Exchange(ref _disposed, 1UL) != 0UL)
                return;
